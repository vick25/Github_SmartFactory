...
-- Hardware Table
INSERT INTO smartnew.`hardware`
SELECT h.HwNo, h.Machine 
FROM smartfactory.`hardware` h 
WHERE h.HwNo <> 0;
...
-- Customlist Table
INSERT INTO smartnew.`customlist`
SELECT c.CustomListNo, c.Code, c.Description 
FROM smartfactory.`customlist` c;
...
-- Configuration Table
INSERT INTO smartnew.`configuration`
SELECT co.ConfigNo, co.Active, co.HwNo, co.AvMinMax, co.ChannelID, co.StartDate, co.EndDate 
FROM smartfactory.`configuration` co
WHERE co.Active = 1;
...
-- Datalog Table 
INSERT INTO smartnew.`datalog`
SELECT d.LogTime, d.ConfigNo, d.LogData
FROM smartfactory.`datalog` d
WHERE d.ConfigNo IN (SELECT co.ConfigNo FROM smartnew.`configuration` co);
...
-- Events Table
INSERT INTO smartnew.`eventlog` (EventTime, UntilTime, CustomListNo, VALUE, HwNo)
SELECT e.EventTime, e.UntilTime, e.CustomCode, e.Value, e.HwNo
FROM smartfactory.`eventlog` e
WHERE e.HwNo IN (SELECT h.HwNo FROM smartNew.`hardware` h)
ORDER BY e.EventNo;
