 ...
 */
public class MachinesProductionTarget extends javax.swing.JDialog {
    ...
    public static void setTargetFound(boolean targetFound) {
        MachinesProductionTarget.targetFound = targetFound;
    }
    ...
    public MachinesProductionTarget(java.awt.Frame parent, boolean modal) throws SQLException {
        super(parent, modal);
                ...
                if (e.getSource() == tableTarget.getSelectionModel() && tableTarget.getRowSelectionAllowed()) {
                    if (tableTarget.getSelectedRow() > -1) {
                        try {
                            ...
                            targetOptions = new MachineTargetParameters(_machine);
                            scrlTargetOptions.setViewportView(targetOptions);
                            _tableTargetOption = targetOptions.getSortableTable();
                            /*Method to show the target options table upon selection on the target
                             *table of a particular machine */
                            setTargetOptionsProductionTime();
                        } catch (SQLException ex) {
                            ConnectDB.catchSQLException(ex);
                        } catch (ParseException ex) {
                            Logger.getLogger(MachinesProductionTarget.class.getName()).log(Level.SEVERE, null, ex);
                        }
                    } else {
...
//                }
        ...
        String targetUnit = ConnectDB.getDBTargetUnit();
        if (null != targetUnit) {
            switch (targetUnit) {
                case "second":
    ...
    private void initComponents() {
        ...
        setTitle("Machines Production Target (min)");

        jPanel1.setBackground(new java.awt.Color(255, 255, 255));

        btnSaveSettings.setBackground(new java.awt.Color(255, 255, 255));
        btnSaveSettings.setIcon(new javax.swing.ImageIcon(getClass().getResource("/images/icons/save.png"))); // NOI18N
        btnSaveSettings.setText("Save Settings");
        btnSaveSettings.setFocusable(false);
        btnSaveSettings.setIconTextGap(8);
...
//                        }
    ...
    private void radSecondActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_radSecondActionPerformed
        this.setTitle("Machines Production Target (second)");
    ...
    private void radMinuteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_radMinuteActionPerformed
        this.setTitle("Machines Production Target (minute)");
        lastTargetUnit = ConnectDB.pref.get(SettingKeyFactory.DefaultProperties.TARGET_TIME_UNIT, lastTargetUnit);
    ...
    private void radHourActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_radHourActionPerformed
        this.setTitle("Machines Production Target (hour)");
        lastTargetUnit = ConnectDB.pref.get(SettingKeyFactory.DefaultProperties.TARGET_TIME_UNIT, lastTargetUnit);
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            @Override
            public void run() {
                MachinesProductionTarget dialog = null;
                try {
                    dialog = new MachinesProductionTarget(new javax.swing.JFrame(), true);
                } catch (SQLException ex) {
                    ConnectDB.catchSQLException(ex);
                }
    ...
    private MachineTargetParameters targetOptions;
